...
# of time. Since, the temperature varies seasonally so the prior might have \mu0 = 
# c(-20, 0, 20) and moderate uncertainty that is, \omega0 = c(0.1, 1, 1). 
...
omega_n <- xtx + omega0
nu_n <- nu0 + n
sigma_n2 <- (nu0*sigma02 + sum(y^2) + t(mu0) %*% omega0 %*% mu0 - t(mu_n) %*% omega_n %*% mu_n)/ nu_n
...
hist(beta_post_draws[,1], main = expression("Histogram of " * beta[0]), breaks = 30, col = "lightblue")
hist(beta_post_draws[,2], main = expression("Histogram of " * beta[1]), breaks = 30, col = "lightgreen")
hist(beta_post_draws[,3], main = expression("Histogram of " * beta[2]), breaks = 30, col = "lightpink")
...
hist(x_min,  breaks = 30, col = "gray", main = "Posterior of time with lowest temp",
     xlab = "Time (x̃)" )
